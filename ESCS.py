...
def lambda_handler(event, context):
    ...
    for database_url in database_urls:
        ...
        df = pd.DataFrame()
        df = (pd.read_sql("SELECT cnt, AVG(CAST (escs as float)) FROM responses WHERE escs != 'NA' GROUP BY cnt", connection_url))
        country = df["cnt"].iloc[0]
        escs = round((df["avg"].iloc[0]), 2)
        if country in countries:
            data_list.append({"id":country,  "value":escs})
    return {
  "datasets": data_list
}
